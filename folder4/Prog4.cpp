...
{
    int count = 1;      //
    int size = 0;       //# of triangles
	double area = 0;    //Variables for output of areas

    if (!ValidPoly(polygon))
		return;

    polygon.Rewind();
    Point pt1 = polygon.CurrentEntry();           //First point or current entry
	polygon.Skip();                             //skip for second point is current entry
	Point ptMid = polygon.CurrentEntry();
	polygon.Rewind();

	while (!polygon.AtEnd())
	{

		double a = 0;			//Triangle side 1
		double b = 0;			//Triangle side 2
		double c = 0;	        //triangle side 3
		double s = 0;           //Avg of sides of triangle
        double Triangle = 0;    //Area if triangle
		polygon.Skip();
		{
			...
			else
			{
                ...
                Point pt3 = polygon.CurrentEntry();

				a = pt1.Length(pt2);
				b = pt2.Length(pt3);                                      //Heron's formula
				c = pt1.Length(pt3);
				s = (a + b + c) / 2;
				Triangle = sqrt(s * (s - a) * (s - b) * (s - c));
				area += Triangle;
				cout << "Triangle " << ++size << " area = " << Triangle << endl;   //Display area of triangle
                ptMid = pt3;
                count++;
	...
	cout << "Polygon area = " << area << endl;   //Triangles added up to make polygon
}
